...
//Programa  proporcionado por el ayudante Vicente Lermanda en el laboratorio 6
...
//se selecciona el algoritmo por la linea de comandos
  switch(atoi(argc[1])){
  ...
  for(int n = n_1; n <= N; n += i){
    ...
    for(int j = 0; j < num_of_experiments; j++){ // se corre el programa varias veces para tener varias muestras de tiempo
    ...
    double mm_avg_time = mm_total_time / num_of_experiments; // se calcula el tiempo promedio de ejecución
